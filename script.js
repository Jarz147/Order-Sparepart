...
function renderTable(data) {
    ...
    const canEdit = ADMIN_EMAILS.includes(currentEmail);

    body.innerHTML = data.map((i, index) => {
        const fotoHtml = i.gambar 
            ? `<img src="${i.gambar}" class="w-10 h-10 object-cover rounded-lg cursor-pointer hover:scale-150 transition-all" onclick="window.open('${i.gambar}')">`
            : `<div class="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-[7px] text-slate-300 italic text-center leading-tight">No Pic</div>`;
        ...
        return `
            <tr class="hover:bg-slate-50 transition-all border-b border-slate-50">
                ...
                <td class="px-6 py-5 text-center">
                    ${canEdit ? `<button onclick="window.openModal('${i.id}')" class="p-2 bg-indigo-50 text-indigo-600 rounded-lg font-bold text-[10px] uppercase px-3">Edit</button>` : '<span class="text-slate-300 text-[8px]">VIEW</span>'}
                </td>
            </tr>
        `;
    }).join('');
...
// --- LOGIKA MODAL (TIDAK ADA EDIT STATUS/PR/PO) ---
window.openModal = (id) => {
...
window.saveAdminUpdate = async () => {
    ...
    const updatedData = {
        ...
        // Status, PR, dan PO tidak disertakan agar tidak berubah di DB
    };
...
window.exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Report Sparepart");
    XLSX.writeFile(wb, `Report_Order_${new Date().toLocaleDateString()}.xlsx`);
};
